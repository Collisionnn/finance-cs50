...
def index():
    ...
    return render_template('index.html', stocks=stocks, cash=usd(cash), usd=usd, total=total)
...
def buy():
    ...
    if request.method == 'POST':
        ...
        if cash < total_price:
            return apology('too poor for this hustle')
        else:
...
def quote():
    ...
    if request.method == 'POST':
        ...
        if not symbol:
            return apology('please enter parameter')
...
def register():
    if (request.method == 'POST'):
        ...
        if not username:
            ...
            return apology('Passowrd is required')
